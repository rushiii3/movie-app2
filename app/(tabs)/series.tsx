...
const Page = () => {
  ...
  const [genresWithSelection, setGenresWithSelection] = useState(
    showGenre?.genres.map((genre) => ({ ...genre, isSelected: false }))
  );
  ...
  const { data, error, isLoading, isFetching, fetchNextPage, hasNextPage } =
    useInfiniteQuery({
      queryKey: ["AllShows", selectedGenres, searchPhrase],
      queryFn: async ({ pageParam = 1 }) => {
        const fetchUrl = searchPhrase 
        ? `https://api.themoviedb.org/3/search/tv?query=${searchPhrase}&language=en-US&page=${pageParam}`
        : `https://api.themoviedb.org/3/discover/tv?language=en-US&page=${pageParam}${selectedGenres ? `&with_genres=${selectedGenres}` : ""}`;
      ...
      {isLoading  ? (
        ...
        <FlashList
          ...
          renderItem={({ item, index }) => (
            <SamllCards item={item} type={"shows"} index={index} />
          )}
